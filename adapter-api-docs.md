...
1 постачальник => 1 адаптер => декілька ендпойнтів (вроді supabase edge functions дозволяють таке)
...
- omega має 4 типи доставки (самовивіз, планува, курєрська, не задано)
- bm - треба:
	- стоврити отримувача 
		- surname – Прізвище
		- name – Ім’я
	...
	- має збереження перевізників
	- має ще щось повязане з курєрами

### 1 - налаштування доставки
- вибір постачальника 
- вибір адреси
- вибір адреси звідки буде доставка
- налаштування отримувача 
Ці дані можна зберігати з нашого боку, у системі, і потім надсилати їх постачальнику коли треба буде. 


### 2 - перегляд статусу доставки
відслідковувати по TTH


## Адаптер
### POST - налаштування доставки
bm - необхідно підтвердження налаштувань доставки по sms 
#### До адаптера йде:
- **receiver** – ID контактної особи - *треба окремо дістати*
- **city** – ID міста - *треба окремо дістати*
- **carrier** – ID перевізника - *треба окремо дістати*
- **carrier_warehouse** – ID складу перевізника - *треба окремо дістати*
- **full_insurance** – Повне страхування. За замовчуванням, `false`
- **is_default** – Встановити налаштування доставки за замовчуванням.
- **note** – Примітка
...
```
#### Помилки:
### GET - відслідковувати доставку
#### До адаптера йде:
- код ТТН
#### Від адаптера
повідомлення про успіх / невдачу
##### Приклад 
якісь дані про доставку
в документації немає прикладів 
#### Помилки:

### GET - отримати дані для налаштувань
більше для omega - там декілька типві доставки і в залежності від замовлення (якщо це так називається) будуть відрізнятися налаштування

для bm це просто треба обєднати декілька запитів в одне (отримати дані про міста, перевізників, отримувачів, областів)
#### До адаптера йде:
- Дані про замовлення (omega - !DocId)

#### Від адаптера
- тип перевезення (якщо є - планова, самовивіз, кур'єр,  не задано)
- постачальник
- дані від апі, які потрібні для створення налаштувань для перевізника 
	- там просто воно у всіх кардинально різне, і я не придумав як то можна об'єднати
	- можна просто з нашого боку зберегти шаблони відповідей для кожного з перевізників / типів доставки
##### Приклад 

#### Помилки:

### ?POST - сотворити отримувача 
Швидше за все, треба буде зберігати з нашого боку, щоб потім підставляти потрібні дані при кидання запитів 
відповідно треба ще для **видалення**.
#### До адаптера йде:
| Query Parameters:                                                                                                                                      |
| ------------------------------------------------------------------------------------------------------------------------------------------------------ |
| - **surname** – Прізвище<br>- **name** – Ім’я<br>- **middle_name** – По батькові<br>- **phone** – Телефон в форматі `380966477711` або `+380956477711` |
##### Приклад
#### Від адаптера
повідомлення про успіх / невдачу
##### Приклад 
